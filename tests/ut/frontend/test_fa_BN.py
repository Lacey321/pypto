...
def fa_k_kernel_bn(
    ...
    with pl.section_cube():
        ...
        left_index = 0
        right_index = 0
        b_start = pl.index_cast(pl.read(b_ranges, [core_id, 0]))
        b_end = pl.index_cast(pl.read(b_ranges, [core_id, 1]))
        n_start = pl.index_cast(pl.read(n_ranges, [core_id, 0]))
        n_end = pl.index_cast(pl.read(n_ranges, [core_id, 1]))
...
def test_fa_k_bn():
    ...
    for b, n, sq, skv, d, num_cores in [
        ...
        # 计算负载均衡的b_ranges和n_ranges
        total_work = b * n
        work_per_core = total_work // num_cores
        extra_work = total_work % num_cores
        ...
        for core in range(num_cores):
            core_work = work_per_core + (1 if core < extra_work else 0)
            start_work = current_work
            end_work = current_work + core_work
            ...
            b_ranges[core, 1] = end_b + 1
            n_ranges[core, 0] = start_n if start_b == end_b else 0
            n_ranges[core, 1] = end_n + 1 if start_b == end_b else n
